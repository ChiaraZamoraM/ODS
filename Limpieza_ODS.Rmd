...
```{r}
#https://www.inei.gob.pe/media/MenuRecursivo/publicaciones_digitales/Est/Lib1829/cap07/cap07043.xlsx
```

8.5.1 Ingreso medio por hora de empleadas y empleados
```{r}
#https://www.inei.gob.pe/media/MenuRecursivo/publicaciones_digitales/Est/Lib1829/cap07/cap07055.xlsx
```
...
# Merge
```{r, warning=FALSE, message=FALSE}
ODS = Reduce(function(x, y) merge(x, y, by= "DEPARTAMENTO",all=TRUE), list(ODS_1,ODS_2, ODS_3, ODS_4, ODS_5, ODS_6, ODS_7))
...
names(ODSSpr)[c(3:ncol(ODSSpr))]= paste0("ODS", names(ODSSpr)[c(3:ncol(ODSSpr))])
...
sheet = createSheet(wb, "ODS 1")

addDataFrame(ODS_1, sheet=sheet, col.names = TRUE, row.names = FALSE)

sheet = createSheet(wb, "ODS 2")

addDataFrame(ODS_2, sheet=sheet, col.names = TRUE, row.names = FALSE)

sheet = createSheet(wb, "ODS 3")

addDataFrame(ODS_3, sheet=sheet, col.names = TRUE, row.names = FALSE)

sheet = createSheet(wb, "ODS 4")

addDataFrame(ODS_4, sheet=sheet, col.names = TRUE, row.names = FALSE)

sheet = createSheet(wb, "ODS 5")

addDataFrame(ODS_5, sheet=sheet, col.names = TRUE, row.names = FALSE)

sheet = createSheet(wb, "ODS 6")

addDataFrame(ODS_6, sheet=sheet, col.names = TRUE, row.names = FALSE)

saveWorkbook(wb, "Base_ODS.xlsx")
```
